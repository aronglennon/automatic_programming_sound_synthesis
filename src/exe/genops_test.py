'''
Generate 100 random patches
Perform each gen op on these patches in 5 different ways
Calculate NSC
'''
...
def store_state(mysql_obj, testrun_id, generation_number, population_data):
    for p in population_data:
        if (mysql_obj.insert_genops_test_data(testrun_id, generation_number, p.patch_to_string(), p.fitness) == []):
